...
public class FieldSelector {
    ...
    private final HashMap<Class, SetMethods> setMethodsCache = new HashMap<>();
    ...
    private Object processCollection(Collection inputValues, Set<String> pathNames) {
        ...
        inputValues.stream().map((value) -> processObject(value, pathNames)).filter((output) -> !(output == null)).forEachOrdered((output) -> {
    ...
    private class PathNames {
        ...
        private final HashSet<String> simplePaths = new HashSet<>();
        private final HashMap<String, HashSet<String>> complexPaths = new HashMap<>();
        ...
        private void buildSimplePaths(Set<String> paths) {
            paths.stream().filter((path) -> !(path == null)).forEachOrdered((path) -> {
