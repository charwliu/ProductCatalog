...
import com.ngboss.eep.catalog.client.CatalogClient;
import com.ngboss.eep.commons.AbstractEntityReference;
 ...
 * id & version fields of entity properties that were of this class. For
 * example, attempting to update or edit the
 * ResourceCandidate.category[n].version would throw an exception. The exception
 * would claim the operation was attempting to update a key field (the real key
 * field is named ENTITY_VERSION in the database). The 'referenced' prefix fixes
 * this issue while making this class a bit uglier than it needs to be.
 *
 */
...
public class CatalogReference extends AbstractEntityReference implements Serializable {
    ...
    @Column(name = "REFERENCED_VERSION")
    ...
    @Column(name = "REFERENCED_HREF")
    @JsonProperty(value = "href")
    private String referencedHref;

    @Column(name = "REFERENCED_NAME")
    @JsonProperty(value = "name")
    private String referencedName;

    @Column(name = "REFERENCED_DESCRIPTION")
    ...
    public boolean equals(Object object) {
        ...
        if (!Utilities.areEqual(this.referencedId, other.referencedId)) {
            return false;
        }

        if (!Utilities.areEqual(this.referencedVersion, other.referencedVersion)) {
            return false;
        }

        if (!Utilities.areEqual(this.referencedHref, other.referencedHref)) {
            return false;
        }

        if (!Utilities.areEqual(this.referencedName, other.referencedName)) {
            return false;
        }

        return Utilities.areEqual(this.referencedDescription, other.referencedDescription);
    }
